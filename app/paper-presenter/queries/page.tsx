...
interface PaperPresenterQuery {
  id: string;
  conferenceId: string;
  question: string;
  category: 'submission' | 'technical' | 'presentation' | 'general';
  reply?: string;
  status: 'open' | 'answered';
  createdAt: string;
}

const mockQueries: PaperPresenterQuery[] = [
  {
    id: '1',
    conferenceId: '1',
    question: 'What are the specific formatting requirements for references?',
    category: 'technical',
    reply: 'Please follow APA 7th edition format. See the submission guidelines document for detailed examples.',
    status: 'answered',
  ...
  },
  {
    id: '2',
    conferenceId: '1',
    question: 'Is there a maximum file size for paper submissions?',
    category: 'submission',
    reply: undefined,
    status: 'open',
...
export default function PaperPresenterQueriesPage() {
  const [queries, setQueries] = useState<PaperPresenterQuery[]>(mockQueries);
  const [isAddingNew, setIsAddingNew] = useState(false);
  const [newQuestion, setNewQuestion] = useState('');
  const [newCategory, setNewCategory] = useState<'submission' | 'technical' | 'presentation' | 'general'>('general');
  ...
  const handleAddQuery = () => {
    ...
    const query: PaperPresenterQuery = {
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-600 mt-1">Ask questions about submission and presentation</p>
          </div>
