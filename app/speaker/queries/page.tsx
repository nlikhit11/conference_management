...
interface SpeakerQuery {
  ...
  conferenceId: string;
  question: string;
  category: 'general' | 'technical' | 'accommodation' | 'logistics';
  reply?: string;
  status: 'open' | 'answered';
  createdAt: string;
}

const mockQueries: SpeakerQuery[] = [
  {
    id: '1',
    conferenceId: '1',
    question: 'What are the technical requirements for my presentation?',
    category: 'technical',
    reply: 'Projector and microphone will be provided. Please bring your presentation in PDF format.',
    status: 'answered',
...
export default function QueriesPage() {
  const [queries, setQueries] = useState<SpeakerQuery[]>(mockQueries);
  const [isAddingNew, setIsAddingNew] = useState(false);
  const [newQuestion, setNewQuestion] = useState('');
  const [newCategory, setNewCategory] = useState<'general' | 'technical' | 'accommodation' | 'logistics'>('general');
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-600 mt-1">Ask questions about your event</p>
          </div>
